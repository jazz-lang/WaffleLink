...
pub const LARGE_SIZE: usize = 0;
pub const SIZE_CLASSES: usize = 6;

pub struct Heap {
    pub size_classes: [Vec<Box<block::HeapBlock>>; SIZE_CLASSES],
    pub start: *mut u8,
}
...
impl Heap {
    ...
    pub fn allocate(&mut self, size: usize) -> Address {
        ...
        for block in self.size_classes[sc].iter_mut() {
            let mem = block.allocate();
            if mem.is_non_null() {
                return mem;
            }
        }
        self.size_classes[sc].push(block::HeapBlock::new(sc));
        self.size_classes[sc].last_mut().unwrap().allocate()
    }
    ...
    pub fn collect(&mut self, sp: Address) {
        ...
        // conservative roots
        {
            ...
            println!("{:p}..{:p}", start.to_ptr::<u8>(), end.to_ptr::<u8>());
            while start < end {
