...
impl Class {
    ...
    pub fn add_property(&mut self, key: Handle<String>) -> Handle<Self> {
        ...
        self.descriptors.as_mut().unwrap().insert(key, Descriptor::Transition(class));
        class
    }
    pub fn has_property(&self,key: &str) -> bool {
        self.descriptors.as_ref().unwrap().iter().find(|x| x.0.get() == key).is_some()
    }
    ...
    pub fn append(&mut self, key: Handle<String>) {
        ...
        self.descriptors.as_mut().unwrap().insert(key, Descriptor::Property(id as _));
    }
    ...
    pub fn clone_class(&self) -> Handle<Self> {
        ...
        for i in 0..self.keys.as_ref().unwrap().len() {
            ...
            class.descriptors.as_mut().unwrap().insert(key, self.descriptors.as_ref().unwrap()[&key]);
        }
...
impl Table {
    pub fn load(&self,key: &str) -> Option<Value> {
        match self {
            ...
            Table::Slow(_,properties) => {
                properties.get(key).copied()
            }
        }
    }
