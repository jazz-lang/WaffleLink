...
thread_local! {
    pub static THREAD: RefCell<Arc<WaffleThread>> = RefCell::new(WaffleThread::with_id(0));
}
...
unsafe impl Send for WaffleThread {}
unsafe impl Sync for WaffleThread {}

/// Waffle native thread.
pub struct WaffleThread {
    pub id: AtomicUsize,
    pub state: StateManager,
    pub tld: ThreadLocalData,
    pub saved_pc: AtomicUsize,
    pub saved_bp: AtomicUsize,
    pub inner: UnsafeCell<WaffleThreadInner>,
}

impl WaffleThread {
    pub fn new() -> Arc<Self> {
        Self::with_id(super::RT.threads.next_id())
    }
    pub fn with_id(id: usize) -> Arc<Self> {
        Arc::new(Self {
            id: AtomicUsize::new(id),
            tld: ThreadLocalData::new(),
            saved_pc: AtomicUsize::new(0),
            saved_bp: AtomicUsize::new(0),
            state: StateManager::new(),
            inner: UnsafeCell::new(WaffleThreadInner {
    ...
    pub fn unpark(&self) {
        if super::RT.threads.safepoint_id() != 0 {
            //safepoint::block(self);
        }
