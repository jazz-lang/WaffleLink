...
pub fn initialize_math(state: &RcState) {
    ...
    cell.add_attribute(Arc::new("sin".to_owned()),Value::from(state.allocate_native_fn(math_sin,"sin",1)));
    cell.add_attribute(Arc::new("cos".to_owned()),Value::from(state.allocate_native_fn(math_cos,"cos",1)));
    cell.add_attribute(Arc::new("tan".to_owned()),Value::from(state.allocate_native_fn(math_tan,"tan",1)));
    cell.add_attribute(Arc::new("asin".to_owned()),Value::from(state.allocate_native_fn(math_asin,"asin",1)));
    cell.add_attribute(Arc::new("acos".to_owned()),Value::from(state.allocate_native_fn(math_acos,"acos",1)));
    cell.add_attribute(Arc::new("atan".to_owned()),Value::from(state.allocate_native_fn(math_atan,"atan",1)));
    cell.add_attribute(Arc::new("atan2".to_owned()),Value::from(state.allocate_native_fn(math_atan2,"atan2",2)));
    cell.add_attribute(Arc::new("exp_m1".to_owned()),Value::from(state.allocate_native_fn(math_exp_m1,"exp_m1",1)));
    cell.add_attribute(Arc::new("ln_1p".to_owned()),Value::from(state.allocate_native_fn(math_ln_1p,"ln_1p",1)));
    cell.add_attribute(Arc::new("sinh".to_owned()),Value::from(state.allocate_native_fn(math_sinh,"sinh",1)));
    cell.add_attribute(Arc::new("cosh".to_owned()),Value::from(state.allocate_native_fn(math_cosh,"cosh",1)));
    cell.add_attribute(Arc::new("tanh".to_owned()),Value::from(state.allocate_native_fn(math_tanh,"tanh",1)));
    cell.add_attribute(Arc::new("asinh".to_owned()),Value::from(state.allocate_native_fn(math_asinh,"asinh",1)));
    cell.add_attribute(Arc::new("acosh".to_owned()),Value::from(state.allocate_native_fn(math_acosh,"acosh",1)));
    cell.add_attribute(Arc::new("atanh".to_owned()),Value::from(state.allocate_native_fn(math_atanh,"atanh",1)));
    cell.add_attribute(Arc::new("random".to_owned()),Value::from(state.allocate_native_fn(math_rand,"random",0)));
    cell.add_attribute(Arc::new("randomInt".to_owned()),Value::from(state.allocate_native_fn(math_rand_int,"randomInt",0)));
