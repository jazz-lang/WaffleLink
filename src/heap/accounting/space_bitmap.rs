...
impl<const ALIGNMENT: usize> SpaceBitmap<ALIGNMENT> {
    ...
    pub const fn index_to_offset(index: usize) -> isize {
        return index as isize * ALIGNMENT as isize * (size_of::<usize>() as isize * 8);
    }
    ...
    pub fn walk(&self, mut visitor: impl FnMut(usize)) {
        unsafe {
            ...
            for i in 0..=end {
                let mut w = (&*bitmap_begin.offset(i as _)).load(Ordering::Relaxed);
                if w != 0 {
                    let ptr_base = Self::index_to_offset(i) as usize + self.heap_begin;
    ...
    pub fn clear_to_zeros(&mut self) {
        if !self.bitmap_begin.is_null() {
            unsafe {
                std::ptr::write_bytes(self.mem_map.as_ptr() as *mut u8, 0, self.mem_map.len());
            }
        }
    }
        ...
        unsafe {
            ...
            for i in 0..count {
                (&*dest.offset(i as _)).store(
                    (&*src.offset(i as _)).load(Ordering::Relaxed),
                    Ordering::Relaxed,
                );
            }
        }
