...
impl FullGenerator for NEqualGenerator {
    fn fast_path(&mut self, gen: &mut FullCodegen) -> bool {
        ...
        gen.masm.set(REG_RESULT, CondCode::NotEqual);
    ...
    fn slow_path(&mut self, gen: &mut FullCodegen) {
        ...
        gen.masm.raw_call(neq as *const u8);
        gen.masm.new_boolean(REG_RESULT,REG_RESULT);
...
extern "C" fn neq(x: Value,y: Value) -> bool {
    !Runtime::compare_equal(x,y)
}