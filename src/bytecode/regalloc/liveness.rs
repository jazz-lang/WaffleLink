...
pub struct InterferenceGraph {
    nodes: LinkedHashMap<ID, Node>,

    adj_set: LinkedHashSet<(ID, ID)>,
    adj_list: LinkedHashMap<ID, LinkedHashSet<ID>>,
    degree: LinkedHashMap<ID, usize>,
...
impl InterferenceGraph {
    ...
    fn new_node(&mut self, reg_id: ID, ty: NodeType, loop_depth: usize) -> ID {
    ...
    fn add_move(&mut self, src: ID, dst: ID) {
        let src = {
    ...
    pub fn add_edge(&mut self, u: ID, v: ID) {
...
/// * uses
/// * defs
fn build_cfg_nodes(cf: &mut Arc<Vec<BasicBlock>>) -> LinkedHashMap<ID, CFGBlockNode> {
    ...
    let (start_inst_map, end_inst_map) = {
        ...
        for block in cf.iter() {
            start_inst_map.insert(block.index, block.index);
            end_inst_map.insert(block.index, block.index);
        }
    ...
    for (id, block) in cf.iter().enumerate() {
        ...
        let (livein, defs) = {
            ...
            let mut all_defined: LinkedHashSet<ID> = LinkedHashSet::new();
            ...
            let defs: Vec<ID> = all_defined.iter().map(|x| *x).collect();
...
fn global_liveness_analysis(
    blocks: LinkedHashMap<ID, CFGBlockNode>,
    ...
    let mut livein: LinkedHashMap<ID, LinkedHashSet<ID>> = {
        let mut ret = LinkedHashMap::new();
    ...
    let mut liveout: LinkedHashMap<ID, LinkedHashSet<ID>> = {
        let mut ret = LinkedHashMap::new();
        for name in blocks.keys() {
            ret.insert(name.clone(), LinkedHashSet::new());
        }
    ...
    for block in blocks.keys() {
        ...
        let liveout: Vec<ID> = liveout
            ...
            .map(|x| *x)
            .collect();
        if let Some(block) = cf.get_mut(*block) {
...
pub fn build_interference_graph_chaitin_briggs(cf: &mut Arc<Vec<BasicBlock>>) -> InterferenceGraph {
        ...
        for i in block.instructions.iter() {
            ...
            if let Instruction::Move { .. } = i {
                for reg_id in i.get_defs() {
                    let reg_id = c(reg_id);
                    ig.new_node(reg_id, Copy, loop_depth);
                }
                ...
                for reg_id in i.get_defs() {
                    let reg_id = c(reg_id);
                    ig.new_node(reg_id, Def, loop_depth);
                }

                for reg_id in i.get_uses() {
                    let reg_id = c(reg_id);
                    ig.new_node(reg_id, Use, loop_depth);
                }
            }
        }
    }
    ...
    for (id, block) in cf.iter().enumerate() {
        ...
        for i in block.instructions.iter().rev() {
            let src: Option<ID> = {
                if let Instruction::Move { .. } = i {
                    let src = i.get_uses();
                    let dst = i.get_defs();
...
fn build_global_liveness(cf: &mut Arc<Vec<BasicBlock>>) {
