...
impl AtomicBitSet {
    ...
    /// ```
    pub fn set(&self, mut position: usize) -> bool {
        assert!(
            position < self.cap,
        ...
        for layer in &self.layers {
            let slot = position / BITS;
            let offset = position % BITS;
            println!("set {}", layer.set(slot, offset));
            position >>= BITS_SHIFT;
        }
        set
    }
...
impl AtomicLayer {
    ...
    /// assert!(!set.is_empty());
    /// ```
    pub fn set(&self, slot: usize, offset: usize) -> bool {
        ...
        // [AtomicBitSet::set].
        self.slot(slot).fetch_or(1 << offset, Ordering::Relaxed) & (1 << offset) > 0
    }
