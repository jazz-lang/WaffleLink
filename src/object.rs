...
pub mod pure_nan {
    ...
    pub fn is_impure_nan(value: f64) -> bool {
        return value.to_bits() >= 0xfffe000000000000u64;
    }
    #[inline]
...
impl Value {
    ...
    #[inline]
    pub fn as_object(self) -> Gc<dyn Object> {
        unsafe { core::mem::transmute(self.payload()) }
    }
    #[inline]
    pub fn as_object_ref(self) -> &Gc<dyn Object> {
...
impl Value {
    ...
    pub fn as_object_ref<'a>(&'a self) -> &'a Gc<dyn Object> {
        unsafe { &self.u.object }
    }
    pub fn is_number(self) -> bool {
...
impl HeapObject for Value {
    fn visit_references(&self, tracer: &mut dyn Tracer) {
        if self.is_cell() {
            self.as_object_ref().visit_references(tracer);
        }
    }
}

pub trait ArrayElement: HeapObject {
    const REF: bool;
}
...
// technically not all values are references but we don't know this ahead of time
// so just assume that every value is reference
...
impl<T: Copy + ArrayElement> Array<T> {
    ...
    pub fn get_at(&self, i: usize) -> T {
        unsafe { self.data().offset(i as _).read() }
    }

    pub fn get(&self, i: usize) -> &T {
        unsafe { &*self.data().offset(i as _) }
    }

    pub fn get_mut(&mut self, i: usize) -> &mut T {
        unsafe { &mut *self.data().offset(i as _) }
    }

    pub fn set_at(&self, i: usize, value: T) {
        unsafe {
            self.data().offset(i as _).write(value);
        }
    }
...
impl<T: Copy + HeapObject + ArrayElement> HeapObject for Array<T> {
    fn heap_size(&self) -> usize {
        std::mem::size_of::<Self>() + (std::mem::size_of::<T>() * self.len())
    }
    fn visit_references(&self, tracer: &mut dyn Tracer) {
        if !T::REF {
            return;
        ...
        for i in 0..self.len() {
            ...
            value.visit_references(tracer);
        }
    }
}

pub trait Object: HeapObject {}

/// Null terminated string type. It does store `length` field so NULL terminator is not
/// required but for C FFI we still keep it to not reallocate string.
pub struct Str {
    length: usize,
    data: [u8; 0],
}

impl HeapObject for Str {
    fn heap_size(&self) -> usize {
        std::mem::size_of::<Self>() + self.length
    }
}
...
impl Str {
    ...
    pub fn c_str(&self) -> &std::ffi::CStr {
        unsafe { std::ffi::CStr::from_ptr(self.data().cast()) }
    }
...
impl<T: ArrayElement + HeapObject + Copy> Object for Array<T> {}
impl Object for Str {}
